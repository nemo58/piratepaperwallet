...
/// Parameters used to generate addresses and private keys. Look in chainparams.cpp (in komodo/src)
/// to get these values. 
...
pub fn params() -> CoinParams {
    CoinParams {
            ...
            zviewkey_prefix  : "zivks".to_string(),
            cointype         : 133
    }
}
...
pub fn generate_wallet(nohd: bool, zcount: u32, user_entropy: &[u8]) -> String {        
    // Get 32 bytes of system entropy
...
mod tests {
    ...
    fn test_vanity() {
        ...
        // Single thread
        let td = json::parse(&generate_vanity_wallet(1, "ar".to_string()).unwrap()).unwrap();
        assert_eq!(td.len(), 1);
        assert!(td[0]["address"].as_str().unwrap().starts_with("zs1ar"));

        // Multi thread
        let td = json::parse(&generate_vanity_wallet(4, "rr".to_string()).unwrap()).unwrap();
        assert_eq!(td.len(), 1);
        assert!(td[0]["address"].as_str().unwrap().starts_with("zs1rr"));
